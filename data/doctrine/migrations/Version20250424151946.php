 ...
 */
final class Version20250424151946 extends AbstractMigration
{
    ...
    public function down(Schema $schema): void
    {
        ...
        SQL);
        $this->addSql(<<<'SQL'
            CREATE TABLE stock (uuid BINARY(16) NOT NULL COMMENT 'Уникальный идентификатор остатка', cabinet_uuid BINARY(16) NOT NULL COMMENT 'Идентификатор кабинета', nomenclature_uuid BINARY(16) NOT NULL COMMENT 'Идентификатор номенклатуры', warehouse_uuid BINARY(16) NOT NULL COMMENT 'Идентификатор склада первичного прихода', purchase_price NUMERIC(15, 2) NOT NULL COMMENT 'Цена прихода на склад', quantity NUMERIC(15, 3) NOT NULL COMMENT 'Количество остатка', created_at DATETIME NOT NULL COMMENT 'Дата и время создания записи', updated_at DATETIME DEFAULT NULL COMMENT 'Дата и время последнего обновления записи', INDEX IDX_stock_warehouse (warehouse_uuid), INDEX IDX_stock_cabinet (cabinet_uuid), INDEX IDX_stock_nomenclature (nomenclature_uuid), PRIMARY KEY(uuid)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_general_ci` ENGINE = InnoDB COMMENT = 'Остатки товаров' 
        SQL);
        $this->addSql(<<<'SQL'
            CREATE TABLE warehouse (uuid BINARY(16) NOT NULL COMMENT 'Уникальный идентификатор склада', cabinet_uuid BINARY(16) NOT NULL COMMENT 'Идентификатор кабинета', name VARCHAR(191) CHARACTER SET utf8mb4 NOT NULL COLLATE `utf8mb4_general_ci` COMMENT 'Название склада', type VARCHAR(50) CHARACTER SET utf8mb4 NOT NULL COLLATE `utf8mb4_general_ci` COMMENT 'Тип склада', identifier VARCHAR(100) CHARACTER SET utf8mb4 NOT NULL COLLATE `utf8mb4_general_ci` COMMENT 'Внутренний идентификатор склада', created_at DATETIME NOT NULL COMMENT 'Дата и время создания записи', updated_at DATETIME DEFAULT NULL COMMENT 'Дата и время последнего обновления записи', INDEX IDX_warehouse_cabinet (cabinet_uuid), PRIMARY KEY(uuid)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_general_ci` ENGINE = InnoDB COMMENT = 'Склады' 
        SQL);
        $this->addSql(<<<'SQL'
